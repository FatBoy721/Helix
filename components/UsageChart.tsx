...
export default function UsageChart({ jobs }: { jobs: HistoryJob[] }) {
  const { bars, total, max } = useMemo(() => {
    const now = new Date();
    const days: { label: string; grams: number }[] = [];
    for (let i = DAYS - 1; i >= 0; i--) {
      const d = new Date(now.getFullYear(), now.getMonth(), now.getDate() - i);
      days.push({ label: `${d.getMonth() + 1}/${d.getDate()}`, grams: 0 });
    }
    ...
    for (const j of jobs) {
      ...
      // filament_used is mm of 1.75mm filament; convert to grams assuming
      // PLA-ish density (1.24). rough but consistent with what fluidd shows.
      const mm = j.filament_used ?? 0;
      const grams = (mm / 1000) * 2.405 * 1.24; // mm -> m -> cm3 (pi*r^2) -> g
