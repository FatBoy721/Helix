...
export const relay = onRequest(async (req, res) => {
  ...
  const title = cleanText(req.headers['x-title'] || body.title, `Helix printer ${event}`);
  const message = cleanText(req.headers['x-message'] || body.body || req.body, 'Printer event received.');
